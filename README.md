...
- **Morphing tab switch** (new): clicking a tab icon no longer snaps
  ...
  cross-morph. The old window is hidden when the animation completes. The
  morph now uses a spring/overshoot curve so the switch has a little bounce.

- **Morphing group / ungroup** (new): grouping and ungrouping animate through
  the same 2D-transformer engine as the tab switch, tuned to be more dramatic
  (bigger overshoot + a soft dissolve that stands in for motion blur):
    - *Group*: the absorbed window **implodes** into the target — it scales
      and translates onto the target's footprint while dissolving to
      transparent, and the target gives a short "welcome" scale pulse. When
      the animation finishes the absorbed window is stacked on the target and
      hidden, exactly as before.
    - *Ungroup*: the leaving window **bursts out** from where it sat in the
      group and springs (with overshoot) to its restore position, sharpening
      from a soft, semi-transparent state to fully opaque. If ungrouping
      reveals another hidden member, that member pops in behind it.
  Ungroup triggered by window *destruction* (`closing`) stays instant, since
  there is no live view left to animate. The soft alpha ramps are a
  dependency-free stand-in for blur; a true per-pixel Gaussian blur would need
  a custom shader transformer.
...
| `group_anim_ms`       | 380     | duration of the group (implode) morph           |
| `ungroup_anim_ms`     | 430     | duration of the ungroup (burst) morph           |
| `tab_spring_k`        | 0.9     | tab-switch overshoot (0 = none, >1.5 = bouncy)  |
| `group_spring_k`      | 1.15    | implode overshoot                               |
| `ungroup_spring_k`    | 1.7     | burst overshoot (the most dramatic)             |
| `parent_pulse_amp`    | 0.07    | target window's "welcome" scale bump on group   |
| `reveal_pop_scale`    | 0.90    | start scale of a member revealed on ungroup     |
| `ungroup_start_alpha` | 0.20    | initial softness of the bursting-out window     |
